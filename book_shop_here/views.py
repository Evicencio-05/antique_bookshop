...
class EmployeeSalesView(LoginRequiredMixin, PermissionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Totals (compute separately to avoid M2M join inflation)
        total_orders = orders_qs.count()
        total_revenue = orders_qs.aggregate(v=Sum("sale_amount"))["v"]
        total_revenue = total_revenue or 0
        total_books_sold = orders_qs.aggregate(v=Count("books"))["v"]
        total_books_sold = total_books_sold or 0

        # Books sold (list)
        sold_books = Book.objects.filter(orders__in=orders_qs).distinct().order_by("-pk")

        # Frequent customers for this employee
        frequent_customers = (
...
class SalesDashboardView(LoginRequiredMixin, PermissionRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Summary metrics (compute separately to avoid M2M join inflation)
        summary_orders = completed.count()
        summary_revenue = completed.aggregate(v=Sum("sale_amount"))["v"]
        summary_revenue = summary_revenue or 0
        summary_books = completed.aggregate(v=Count("books"))["v"]
        summary_books = summary_books or 0
        summary_discount = completed.aggregate(v=Sum("discount_amount"))["v"]
        summary_discount = summary_discount or 0
        ...
        # Top employees by books sold and revenue
        top_employees = list(
        ...
        # Top authors by books sold (via M2M)
        top_authors = list(
